...
<div className='w-full  h-max  rounded-lg pb-20'>
...
<div key={item._id} className='w-full flex justify-center items-center text-center border border-fuchsia-400 py-4 my-2 rounded-lg bg-sky-200 '>
...
<button onClick={()=>showInfosHandler(item._id ,item.name , item.last_name , item.username , item.email , item.phone_number , item.country_name , item.state_name , item.city_name , item.address , item.brand_name , item.postal_code , item.work_phone , item.registration_date )} className="bg-indigo-300 py-1 w-max px-2  rounded-md" >show information</button>
</div>
...
: <p className='text-center pt-36 text-xl'>there is no request</p>}
