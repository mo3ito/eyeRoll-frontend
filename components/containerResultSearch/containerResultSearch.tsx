...
export default function ContainerResultSearch() {
    ...
    const changeInputValueHandler = (event : ChangeEvent)=>{
    ...
    useEffect(()=>{
        if(inputValue){
                information.map(item=>{
                        if(item.name.includes(inputValue) && !resultSearch.some(resultItem=> resultItem?.name === item.name )  ){
                           setResultSearch(prev=>[...prev , item])
                      }
...
<div className='w-full relative'>
	...
	<div className='w-full h-max bg-sky-50 border border-indigo-200 shadow-lg '>

	<ul className='w-full  h-max max-h-72 overflow-y-auto p-1 text-sm md:text-base xl:text-lg '>
	<li className='w-full px-1 md:px-2  max-xs:h-7 h-10 md:h-12 max-xs:pt-1 pt-[10px]   hover:bg-blue-200 hover:opacity-100 overflow-hidden  cursor-pointer opacity-70 truncate'>
