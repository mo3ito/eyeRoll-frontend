...
export default function MoblilMode() {
  ...
  return (
    ...
    <div  className={`${ showProfile ? 'absolute' : 'hidden'} w-36 h-max bg-blue-100 text-base z-50 shadow-md border border-purple-500 rounded-md`}>
      <div className='flex items-center  px-1 border-b border-purple-500 py-2 relative'>
      ...
      <p className='truncate  ml-1 border-purple-500   '>{!infos?.is_businessOwner && infos?.username && infos?.is_verified ? infos?.username : "no login" }</p>
      </div>
