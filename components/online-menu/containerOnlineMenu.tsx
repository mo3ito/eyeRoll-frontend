...
export default function ContainerOnlineMenu({sortedProduct = [] , setIsShowProduct , setProductDetails}: ContainerOnlineMenuProps) {
  ...
  const detailsHandler = (producName: string , productImage: string , productPrice: string , productPricePetty: string , productDescription:string , productAssortment:string )=>{
 ...
 {sortedProduct.map((item : sortedProduct) =>
           ...
           {item.values.map((product : ProductType) =>
               <div onClick={()=>detailsHandler(product.productName , product.product_image_path , product.productPrice , product.productPricePetty , product.productDescription , product.productAssortment )} key={product._id} 
               ...
               {product.productDescription && <button onClick={()=>detailsHandler(product.productName , product.product_image_path , product.productPrice , product.productPricePetty , product.productDescription , product.productAssortment )} className='truncate text-start w-full opacity-40 hover:underline'> <span className='truncate'>details: </span>{product?.productDescription}</button>}
