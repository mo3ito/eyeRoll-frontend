...
export default function ContainerFilterMenu({groupName , filteredProduct , setIsShowProduct , setProductDetails }:ContainerFilterMenuProps) {
  ...
  return (
   ...
   <div className=' w-[100px] sm:w-6/12 md:w-4/12 lg:w-5/12 h-full '>
    ...
    {item.productDescription && <button onClick={()=>detailsHandler(item.productName , item.product_image_path , item.productPrice , item.productPricePetty , item.productDescription , item.productAssortment )} className='truncate text-start w-full opacity-40 hover:underline'> <span className='truncate'>details: </span>{item?.productDescription}</button>}
