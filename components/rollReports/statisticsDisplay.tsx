...
export default function StatisticsDisplay() {
  ...
  const [socketEyeRoll, setSocketEyeRoll] = useState<Socket | null>(null);
  const [seenUserEyeRollPage, setSeenUserEyeRollPage] = useState<number>(0);
  useEffect(() => {
    const newSocket = io("http://localhost:5002");
    setSocketEyeRoll(newSocket);
    return () => {
  ...
  useEffect(() => {
    if (socketEyeRoll) {
      ...
      socketEyeRoll.on("updateSeenUser", (updatedSeenUser) => {
        setSeenUserEyeRollPage(updatedSeenUser);
      });
    }
  }, [socketEyeRoll]);

  console.log("seenUserEyeRollPage:", seenUserEyeRollPage);

  const [socketOnlineMenu , setSocketOnlineMenu]=useState<Socket|null>(null)
  const [seenUserOnlineMenuPage, setSeenUserOnlineMenuPage] = useState<number>(0);

  useEffect(()=>{
    const onlineMenuSocket = io("http://localhost:5001")
    setSocketOnlineMenu(onlineMenuSocket)

    return ()=>{
      socketOnlineMenu?.disconnect()
    }
  ...
  useEffect(()=>{
    ...
    if(socketOnlineMenu){
      socketOnlineMenu.on("updateSeenUser",(updateSeenUser)=>{
        setSeenUserOnlineMenuPage(updateSeenUser)
      })
    }
  ...
  return (
    ...
    <div className='text-4xl font-semibold'>{seenUserEyeRollPage}</div>
    ...
    <p className='w-full h-max text-center font-semibold'>view my EyeRoll page</p>
    </div>

    <div className=' max-xs:w-full w-44   lg:w-56 xl:w-72 xl:h-[330px]  bg-indigo-100 cursor-pointer rounded-2xl hover:scale-105 duration-200 flex items-center justify-around  flex-col py-10 px-4'>
    <div className='text-4xl font-semibold'>{seenUserOnlineMenuPage}</div>
    ...
    <p className='w-full h-max text-center font-semibold'>view my online menu page</p>
    </div>
