...
export default function MoblilMode() {
  ...
  return (
    ...
    { infos?.username && !infos.is_businessOwner && <div className='relative '>
    <div ref={profileRef} onClick={()=>setShowProfile(prev=> !prev)} className='w-12 mt-4 h-12 rounded-full flex items-center justify-center  bg-indigo-400 '>
    <img src="/images/defaultPerson.png" alt="" />
    </div>
    <div  className={`${ showProfile ? 'absolute' : 'hidden'} w-36 h-max bg-blue-100 absolute text-base z-50 shadow-md border border-purple-500 rounded-md`}>
      ...
      <p className='truncate  ml-1 border-purple-500   '>{infos.username}</p>
      </div>
     
    <ul className="   rounded-b-md p-1 border-purple-500 bg-blue-100 z-50 shadow-md  h-max   w-full  px-1 ">
