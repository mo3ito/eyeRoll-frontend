...
<>
    ...
    <div className='flex items-center   text-xl h-full space-x-3  w-full '>
        ...
        <button
          className="iniline-block text-xl   sm:mr-8"
        ...
        <svg className='w-4 h-4 fill-zinc-700 inline-block' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M12 16L6 10H18L12 16Z"></path></svg>
          ...
          <ul className={`hidden ${ showBox && 'group-hover:block'} text-sm sm:text-lg absolute border p-1 border-purple-500 bg-blue-100 z-50 shadow-md  top-24 left-0 h-max w-52 sm:min-w-[176px] sm:w-max  rounded-xl `}>
            ...
            { showSwitchAccount && <ul className="absolute  hidden group-hover/item:block top-8 w-44 sm:left-full sm:top-0 sm:min-w-xl sm:max-w-max border border-purple-500 bg-blue-100 p-1  rounded-lg">
              <li className=" px-2 sm:px-4 py-2  hover:bg-pink-300 rounded-lg text-fuchsia-700 hover:font-semibold hover:text-white">mostafa</li>
              <li className=" px-2 sm:px-4 py-2  hover:bg-pink-300 rounded-lg text-fuchsia-700 hover:font-semibold hover:text-white">saj</li>
              <li className=" px-2 sm:px-4 py-2  hover:bg-pink-300 rounded-lg text-fuchsia-700 hover:font-semibold hover:text-white">rez</li>
            </ul>}
              </li>
                <li onClick={()=>logOutHandler(router)} className=' px-2 sm:px-6 py-2  hover:bg-pink-300 rounded-lg text-fuchsia-700 hover:font-semibold hover:text-white '>
            log out
              </li>
          </ul>
